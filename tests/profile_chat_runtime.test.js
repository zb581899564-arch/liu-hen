    ...
    'sticker_profile.json': JSON.stringify({
      ...
      high_frequency_md5: [

        {
          ...
          path: 'G:\\ai\\llm\\鍓嶄换skill\\鑱婂ぉ璁板綍\\emojis\\abc.gif',
...
test('windowsPathToFileUrl converts local sticker path to file url', () => {
  ...
  const fileUrl = windowsPathToFileUrl('D:\\profiles\\sample-contact\\emojis\\abc.gif');

  assert.equal(
    fileUrl,
    'file:///D:/profiles/sample-contact/emojis/abc.gif'
  );
...
test('createProfileBundleFromFiles reads browser File objects', async () => {
  ...
  const files = [
    ...
    new File([JSON.stringify({
      ...
      high_frequency_md5: [{ md5: 'abc', count: 12, path: 'G:\\ai\\llm\\鍓嶄换skill\\鑱婂ぉ璁板綍\\emojis\\abc.gif' }],
