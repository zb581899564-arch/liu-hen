...
test('chat persistence can use Android bridge when localStorage quota fails', () => {
  ...
  try {
    saveChatPersistence({
      enabled: true,
      messages: {
        'permanent-sample-contact': [{ kind: 'message', role: 'assistant', text: '还在吗' }],
      },
    });

    assert.match(savedPayload, /permanent-sample-contact/);
    assert.deepEqual(loadChatPersistence().messages['permanent-sample-contact'], [
      { kind: 'message', role: 'assistant', text: '还在吗' },
    ]);
  } finally {
    global.localStorage = previousStorage;
    global.AndroidBridge = previousBridge;
...
test('chat persistence falls back to existing local data before native store has content', () => {
  ...
  local.setItem('wechatProfileChatPersistence', JSON.stringify({
    enabled: true,
    messages: {
      'permanent-sample-contact': [{ kind: 'message', role: 'assistant', text: '旧记录' }],
    },
  }));
  global.AndroidBridge = {
    loadChatPersistence() {
  ...
  try {
    assert.deepEqual(loadChatPersistence().messages['permanent-sample-contact'], [
      { kind: 'message', role: 'assistant', text: '旧记录' },
    ]);
  } finally {
    global.localStorage = previousStorage;
    global.AndroidBridge = previousBridge;
    ...
    saveMoments({
      ...
      postsBySlug: {
        'permanent-sample-contact': [{
          ...
          createdAt: 100,
          likedByUser: true,
          comments: [{ id: 'comment-1', authorRole: 'user', text: '那你多穿点', createdAt: 120 }],
        }],
      },
      nextPostAtBySlug: {
    ...
    assert.deepEqual(loadMoments(), {
      ...
      postsBySlug: {
        'permanent-sample-contact': [{
          ...
          createdAt: 100,
          likedByUser: true,
          comments: [{ id: 'comment-1', authorRole: 'user', authorSlug: '', text: '那你多穿点', createdAt: 120 }],
        }],
      },
      nextPostAtBySlug: {
        'permanent-sample-contact': 1000,
